...
1. Set `.env`
    ...
    Copy the file content `https://github.com/tokenlin/monad-demo/blob/main/packages/foundry/.env.example`, and replace the content of the local file `packages/foundry/.env`.
 
    We need to Add a private key prefixed with '0x' to `DEPLOYER_PRIVATE_KEY`, and RPC to `MONAD_RPC`.
...
2. `monad`: Open `Setting` on Metamask Wallet, choose `Networks`, and click `Add a network`, enter the following infos:
    ```
    Network name: Monad
    New RPC URL: https://xxx (enter the Monad RPC URL)
    Chain ID: 41454
    Currency symbol: MON
